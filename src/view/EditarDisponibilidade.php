      ...
      // inclui o arquivo de menu responsivo no sistema
        ...
        <section> 
            <div class="row-fluid">
                    <div id="conteudo">
                            <div class="span12">
                                <form id="curso" class="form-horizontal" method="POST" action="index.php?pg=AtualizarDisponibilidade">
                                        <fieldset>
                                        ...
                                        <table class="table">
                                            ...
                                            <?php for ($i = 1; $i < 7; $i++) {
                                                echo '<tr><td>'.$dia[$i].'</td>';
                                                for ($j = 1; $j < 16; $j++) { 
                                                    for ($k = 0; $k < count($verifica); $k++) {
                                                        
                                                            if(($verifica[$k]->hopr_dia == $i) AND ($verifica[$k]->hopr_aula == $j)){
                                                                echo '<td><input id="" type="checkbox" ';if($verifica[$k]->hopr_status == 1) {echo 'checked = checked';} if($verifica[$k]->hopr_id_disciplina != NULL){echo 'DISABLED'; } echo ' name="'.$dia[$i].'['.$j.']" value="1" ></td>';
